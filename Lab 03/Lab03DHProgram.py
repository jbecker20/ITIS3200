...
class SecurePRNG:
    ...
    def generate(self, n_bytes):
        ...
        while len(output) < n_bytes:
            ...
            keystream_block = hashlib.sha256(b"keystream:" + self.state).digest()
            output += keystream_block
            # 2. Update state immediately after with a hash function (One-way progression)
            self.state = hashlib.sha256(b"nextstate:" + self.state).digest()
...
class Mallory:
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, bytes):
            ...
            plaintext = plaintext.replace(b"9pm", b"5am")
            # Then use the PRNG shared with bob to re-encrypt and return the message for Bob
            return xor_crypt(plaintext,self.bob_prng)
        return payload
...
def main():
    ...
    message = b"This is a test message" # Put in your test message here
